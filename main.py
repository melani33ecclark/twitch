...
twitch_miner = TwitchChannelPointsMiner(
    ...
    logger_settings=LoggerSettings(
        ...
        telegram=Telegram(                                                          # You can omit or set to None if you don't want to receive updates on Telegram
            ...
            token="123456789:2",                          # Telegram API token @BotFather
            events=[Events.STREAMER_ONLINE, Events.STREAMER_OFFLINE,
                    Events.BET_LOSE, Events.CHAT_MENTION],                          # Only these events will be sent to the chat
            disable_notification=True,                                              # Revoke the notification (sound/vibration)
        ),
        discord=Discord(
            webhook_api="https://discord.com/api/webhooks/1323093320096026735/TsfjDlKbm13TJMn08OLzxFS5F0n4rIIQIzZKu-6mhWXnGUhabD-y9AGODPOOpMHOVPIZ",  # Discord Webhook URL
            events=[Events.STREAMER_ONLINE, Events.STREAMER_OFFLINE,
                    Events.BET_LOSE, Events.CHAT_MENTION, Events.BONUS_CLAIM, Events.GAIN_FOR_WATCH,
...
twitch_miner.mine(
    [
        Streamer("Runah", settings=StreamerSettings(make_predictions=False , follow_raid=True  , watch_streak=True)),   
         Streamer("kaicenat", settings=StreamerSettings(make_predictions=False , follow_raid=True  , watch_streak=True)), 
